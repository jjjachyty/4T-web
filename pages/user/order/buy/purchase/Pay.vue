<template>
    <v-app>
                  ...
                  <div v-if="orders.length < 1" class="text-xs-center"><small class="grey--text">暂无待付款的订单</small></div>
                    ...
                    <v-layout row wrap >
                        ...
                        <v-flex xs12 >
                           <v-layout row  justify-center align-center>
                            ...
                            <v-flex xs5 md8>{{order.seller}}<v-icon small>keyboard_arrow_right</v-icon></v-flex>  
                            <v-flex xs3> <span class="caption font-weight-bold">代购费<small class=" deep-orange--text">¥{{order.charge}}</small></span></v-flex>  
...
<!-- </router-link> -->
                        <v-flex xs12>
                            ...
                            <v-card-actions>
                                                         <v-spacer></v-spacer>
                                                         <v-btn color="grey" outline small @click="showCancel = !showCancel">取消订单<v-icon small v-if="showCancel">keyboard_arrow_down</v-icon><v-icon small v-else>keyboard_arrow_up</v-icon></v-btn>
...
export default {
  ...
  methods: {
    cancel (index) {
      ...
      this.$http.putJson('/usr/order/' + order.id, {id: order.id, state: '5', cancelReason: this.reason}).then(res => {
        if (res.data.Status) {
